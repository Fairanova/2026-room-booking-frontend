...
function switchAdminTab(tab) {
  ...
  document
    .querySelectorAll("#main-content button.btn-outline")
    .forEach((btn) => {
                        ...
                        ${bookings
                          .map(
                            ...
                            <tr>
                                ...
                                <td>
                                    ${(() => {
                                      // Ensure status is handled as integer
                                      const status = parseInt(booking.status);
                                      if (status === BOOKING_STATUS.PENDING) {
                                        return `
                                                <div style="display: flex; gap: 0.5rem;">
                                                    <button class="btn btn-sm btn-primary" onclick="updateBookingStatus('${booking.id}', ${BOOKING_STATUS.APPROVED})">
                                                        ✅ Approve
                                                    </button>
                                                    <button class="btn btn-sm btn-outline" style="color: var(--error); border-color: var(--error);" onclick="updateBookingStatus('${booking.id}', ${BOOKING_STATUS.REJECTED})">
                                                        ❌ Reject
                                                    </button>
                                                </div>
                                            `;
...
async function updateBookingStatus(id, status) {
  ...
  try {
    ...
    showSuccess(
      `Booking ${status === BOOKING_STATUS.APPROVED ? "approved" : "rejected"} successfully`,
    );
    await loadAdminBookings();
  } catch (error) {
...
async function openRoomModal(roomId = null) {
  ...
  document
    ...
    .addEventListener("submit", handleRoomSubmit);
}
